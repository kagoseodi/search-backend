...
/**
 * Search API Endpoint with Strict Relevance Scoring & Ranking
 */
app.get('/api/search', async (req, res) => {
  ...
  // 1. Accumulate match scores from inverted index
  queryTerms.forEach(term => {
    if (invertedIndex.has(term)) {
      invertedIndex.get(term).forEach(docId => {
  ...
  // Helper function to rank documents by search term relevance
  const scoreAndRankDocs = (scoredDocMap) => {
    const scoredList = [];

    scoredDocMap.forEach((baseScore, docId) => {
      ...
      // Extra weight for direct matches in title, URL, or snippet
      queryTerms.forEach(term => {
      ...
      scoredList.push({ doc, score });
    });

    return scoredList.sort((a, b) => b.score - a.score).map(item => item.doc);
  };

  let results = scoreAndRankDocs(docScores);

  // 2. If fewer than 3 local results exist, perform a live web fetch
  if (results.length < 3) {
    ...
    results = scoreAndRankDocs(docScores);
  }
