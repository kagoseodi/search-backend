...
const ServerBackendPipeline = {
    ...
    queryPipelineSearch: async function(query, userId) {
        ...
        // 1. SPECIFIC YOUTUBE HANDLING
        if (cleanQuery.includes('youtube') || cleanQuery.includes('yt') || cleanQuery.includes('video')) {
            hits.push({
                id: `yt-home-${Date.now()}`,
                title: "🔴 YouTube - Video Streaming Platform",
                url: "https://www.youtube.com/",
                snippet: "Main YouTube portal. Access millions of videos, channels, and playlists. Browse trending content, subscriptions, and recommendations.",
                source: "youtube",
                embedUrl: "https://www.youtube.com/embed"
            });
        }

        // 2. DYNAMIC INTERNET ARCHIVE PUBLIC DOMAIN MOVIES (ONLY WHEN MATCHED OR FILTERED)
        const archiveMovies = [
            { title: "🎬 Night of the Living Dead (1968)", id: "night_of_the_living_dead", url: "https://archive.org/details/night_of_the_living_dead", embedUrl: "https://archive.org/embed/night_of_the_living_dead", snippet: "George A. Romero's seminal horror masterpiece. Full public-domain feature film." },
            { title: "🎬 The Phantom of the Opera (1925)", id: "phantom_of_the_opera_1925", url: "https://archive.org/details/phantom_of_the_opera_1925", embedUrl: "https://archive.org/embed/phantom_of_the_opera_1925", snippet: "Lon Chaney stars as the phantom in this iconic 1925 classic silent horror film." },
            { title: "🎬 Charade (1963)", id: "Charade_1963", url: "https://archive.org/details/Charade_1963", embedUrl: "https://archive.org/embed/Charade_1963", snippet: "Audrey Hepburn and Cary Grant star in this legendary romantic mystery thriller." },
            { title: "🎬 House on Haunted Hill (1959)", id: "house_on_haunted_hill", url: "https://archive.org/details/house_on_haunted_hill", embedUrl: "https://archive.org/embed/house_on_haunted_hill", snippet: "Vincent Price invites guests to stay in a spooky mansion for $10,000." },
            { title: "🎬 His Girl Friday (1940)", id: "HisGirlFriday", url: "https://archive.org/details/HisGirlFriday", embedUrl: "https://archive.org/embed/HisGirlFriday", snippet: "Howard Hawks' fast-talking screwball comedy starring Cary Grant and Rosalind Russell." }
        ];

        archiveMovies.forEach((m, idx) => {
            if (cleanQuery.includes('movie') || cleanQuery.includes('film') || cleanQuery.includes('horror') || m.title.toLowerCase().includes(cleanQuery) || m.snippet.toLowerCase().includes(cleanQuery)) {
                hits.push({
        ...
        // 3. VIMEO CINEMA (FILTERED BY MATCH)
        const vimeoMovies = [
            { title: "🎥 Sintel (Open Cinema Film)", vimeoId: "15247292", snippet: "Award-winning open-source fantasy short film produced by the Blender Foundation." },
            { title: "🎥 Tears of Steel (Sci-Fi Movie Short)", vimeoId: "35401002", snippet: "VFX open-source sci-fi short movie set in a dystopian future Amsterdam." },
            { title: "🎥 In the Shadow of the Mountain", vimeoId: "76979871", snippet: "Cinematic documentary short film exploring high-altitude mountain landscapes." }
        ];

        vimeoMovies.forEach((v, idx) => {
            if (cleanQuery.includes('movie') || cleanQuery.includes('vimeo') || cleanQuery.includes('film') || v.title.toLowerCase().includes(cleanQuery)) {
                hits.push({
        ...
        } catch { console.log("Server Pipeline: Archive.org live search dropped."); }

        // 5. GOOGLE SEARCH NODES
        const clusters = ["Core Results", "Global Trends", "Developer Matrix", "Scholar Network"];
        clusters.forEach((cluster, idx) => {
            hits.push({
                id: `g-node-${idx}-${Date.now()}`,
                title: `${term} - Google ${cluster}`,
                url: `https://www.google.com/search?igu=1&q=${encodeURIComponent(query)}`,
                snippet: `Live search data pipeline for "${query}". Opens operational search canvas inside system viewport.`,
                source: "google",
        ...
        // 7. YOUTUBE VIDEO SEEDING & FILTERING
        const youtubeVideoDatabase = [
            { title: "Stranger Things - Official Trailer", videoId: "b9EkMc79ZSU", snippet: "Official series trailer. Embedded video viewport node." },
            { title: "Wednesday Addams - Official Teaser", videoId: "Di310WS8zLk", snippet: "Netflix release teaser. Structural stream index data." },
            { title: "JavaScript Tutorial for Beginners", videoId: "dQw4w9WgXcQ", snippet: "Complete programming tutorial series with direct framing." },
            { title: "Web Development Complete Course", videoId: "9bZkp7q19f0", snippet: "Full course tutorial series for modern frontend development." }
        ];
        ...
        // 8. SOCIAL MATRIX (INSTAGRAM / PROJECT NODES)
...
function processLiveSuggestions(inputElementId, dropdownId) {
    ...
    debounceTimer = setTimeout(() => {
        ...
        // SAFE EVALUATION ENGINE FOR MATHEMATICAL EXPRESSIONS
        ...
        const currencyRegex = /^(\d+(?:\.\d+)?)\s*([A-Za-z]{3})\s+to\s+([A-Za-z]{3})$/i;
            ...
            if (place.length > 0) {
                ...
                segmentHTML += `
                    <div class="suggestion-section">
                        ...
                        <div class="quick-action-card suggestion-nav-target" onclick="selectSuggestionToken('${inputElementId}', '${dropdownId}', 'weather in ${encodeURIComponent(place)}')">
                            <div class="quick-action-main">
        ...
        if (identifiedCategory) {
            ...
            categoryChipsMap[identifiedCategory].forEach(chip => {
                segmentHTML += `<div class="suggestion-chip suggestion-nav-target" onmousedown="executeChipQuery('${inputElementId}', '${dropdownId}', '${encodeURIComponent(rawVal.trim() + ' ' + chip.toLowerCase())}')">${chip}</div>`;
            });
            segmentHTML += `</div></div>`;
        }
    ...
    if (predictedTarget) {
        segmentHTML += `
            <div class="suggestion-section">
                ...
                <div class="suggestion-item suggestion-nav-target" style="background:var(--hover-bg);" onmousedown="selectSuggestionToken('${inputElementId}', '${dropdown.id}', '${encodeURIComponent(predictedTarget.recommendation)}')">
                    <div class="suggestion-item-main">
                        <span class="suggestion-item-icon">✨</span>
...
function executeChipQuery(inputId, dropdownId, encodedEvaluationQuery) {
    const evaluationQuery = decodeURIComponent(encodedEvaluationQuery);
...
async function runPipelineSearch(query, isLucky = false) {
    ...
    if (!isLucky && resultsMain) {
        ...
        if (pipelineStatus) pipelineStatus.innerText = "Status: Server querying indexes...";
    }
    ...
    if (!isLucky) {
        if (logPanel) logPanel.innerHTML = "<div>[Express/Node] Intercepted request headers.</div><div>[Backend] Applied search term filters.</div><div>[Backend] Applied igu=1 bypass configurations.</div><div>[Server] Data payloads assembled successfully.</div>";
        if (pipelineStatus) pipelineStatus.innerText = "Status: Framed index arrays ready.";
    }
